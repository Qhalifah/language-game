    ...
    if(MyRect::m_selectedRect)
    {
        ...
        if(m_edType == ScreenQGV::MAP)
        {
            ...
            else
            {
                ...
                MainWindow::ui->selReqBadge->setCurrentIndex(0);
            }
        }
        else if(m_edType == ScreenQGV::SCENE)
        {
            ...
            else
            {
                if(m_rewardBadgeId)
                {
                    ...
                    MainWindow::ui->actPieceImg->setPixmap(QPixmap(sel->actPieceFilepath()).scaled(50, 50,
                                                                               Qt::KeepAspectRatioByExpanding,
                                                                               Qt::FastTransformation));
                }
                if(sel->gameType() == GameType::PAIR)
                {
                    QString actName("Pairing - ");
        ...
        if(sel->hasSound())
        {
            ...
            MainWindow::ui->sndPitchWidg->setHidden(false);
            MainWindow::ui->sndVol->setValue(sel->volume());
            MainWindow::ui->sndPitch->setValue(sel->pitch());
        }
        else
        {
